...
bool esValida(const vector<int>& super, const vector<bool>& prods, int pos, int i) {
	...
	if (super[i] >= 3) return false; //Si has cogida ya 3 productos en un supermercado, no puedes coger m�s de �l
	return true;
}
	...
	for (int i = 0; i < numSuper; i++) {
		if (esValida(super, prods, cont, i)) {
			...
			if (cont == numProds - 1) {
				...
				//Si el precio que se lleva hasta entonces + lo posible m�s peque�o a partir de ah� es menor que el guardado en total entonces se llama recursivamente, si no, se mantiene dicho total, y ahorras recursion
...
void resuelveCaso() {
	...
	//Obtienes los m�nimos de cada producto
	...
	//Con acumulados voy a obtener la acumulacion del m�s peque�o posible en cada una de las columnas, de la �ltima a la primera
